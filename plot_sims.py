...
def get_fig_axs():
    a = 3
    b = 2
    fig, axs = plt.subplots(a, b,figsize=(10, 6), sharex=True)
    plt.subplots_adjust(left=0.12, bottom=0.1, right=.98, top=0.92, wspace=0.15, hspace=0.08)
...
def plot_trial(triaxial_bool,fig,axs,data,ds,alpha,inds,clr='black',lw=0.75):
    ...
    # r is vector from planet to star -- maybe not for GRIT
    year = 0.4**(3/2)
    ...
    ay = data[inds['ay'],::ds]
    az = data[inds['az'],::ds]
    ts = data[inds['t'],::ds] / 1.e6 / year

    # n = np.sqrt(np.dot(vs[:,0],vs[:,0])) / np.sqrt(np.dot(rs[:,0],rs[:,0])) # mean-motion

    omega_to_ns = np.sqrt(sx**2 + sy**2 + sz**2)
    axis_mags = np.sqrt(ax**2 + ay**2 + az**2)
    thetas = np.degrees(np.arccos(sz/omega_to_ns))
    # phis = np.degrees(phi_rad)
    # psis = np.degrees(sops.get_psi_v2(rs,iss,js,ks,ss))

    betas = np.degrees(np.arccos((ax*sx+ay*sy+az*sz)/omega_to_ns/axis_mags))
    ...
    if triaxial_bool:
        ...
        col = 1
    axs[0,col].plot(ts,omega_to_ns/24.8364706645,color=clr,lw=lw,alpha=alpha)
...
def plot_traj(data, ax, inds):
    ...
    ax.plot(omega_to_ns/24.8364706645,thetas, lw=1., color='black', alpha=0.2)
    ax.scatter(omega_to_ns[:1]/24.8364706645,thetas[:1], s=2, color='tab:red')
    ax.scatter(omega_to_ns[-1:]/24.8364706645,thetas[-1:], s=2, color='tab:blue')
...
if __name__=="__main__":
    ...
    plots_dir = os.path.join("se_res/plots",dir_name)
    if not os.path.exists(plots_dir):
        os.mkdir(plots_dir)
    triax_fs = ["se_res/data/"+dir_name+"/trial_"+str(i)+".npy" for i in range(n_trials)]
    j2_fs = ["se_res/data/"+dir_name+"/trial_"+str(i)+".1.npy" for i in range(n_trials)]
    ...
    if together:
        ...
        plt.subplots_adjust(left=0.12, bottom=0.15, right=.98, top=0.92, wspace=0.02, hspace=0.02)
